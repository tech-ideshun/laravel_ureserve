...
class Calendar extends Component
{
    ...
    public function mount()
    {
        ...
        $this->events = EventService::getWeekEvents($this->currentDate->format('Y-m-d'), $this->sevenDaysLater->format('Y-m-d'));
        // dd($this->events);
        for($i = 0; $i < 7; $i++){
    ...
    public function getDate($date)
    {
        ...
        $this->currentWeek = [];
        $this->sevenDaysLater = CarbonImmutable::parse($this->currentDate)->addDays(7);  // Carbon::parseでインスタンス化しないといけない

        $this->events = EventService::getWeekEvents($this->currentDate, $this->sevenDaysLater->format('Y-m-d'));
